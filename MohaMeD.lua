...
redis:set('Matthew:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('Matthew:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MatthewTEAM')..')\n\n🔖┇تم رفعة مطور في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
redis:set('Matthew:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('Matthew:'..bot_id..'dev', apmd[2])
...
if not redis:sismember('Matthew:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'بالفعل تم تنزيلة من مطورين البوت')
else
redis:del('Matthew:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('Matthew:'..bot_id..'dev', result.sender_user_id_)
...
if result.id_ then
redis:del('Matthew:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('Matthew:'..bot_id..'dev', result.id_)
...
redis:del('Matthew:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('Matthew:'..bot_id..'dev', apmd[2])
...
os.execute('cd libs;wget https://raw.githubusercontent.com/matttewabas/Matthew/master/libs/utils.lua')
os.execute('wget https://raw.githubusercontent.com/matttewabas/Matthew/master/Matthew.lua')
dofile('Matthew.lua')  
return false
end
...
local gps = database:scard('Matthew:'..bot_id.."groups")  
...
redis:del('Matthew:'..bot_id..'name_bot')
...
local gps = database:scard('Matthew:'..bot_id.."userss")  
...
local pv = database:smembers('Matthew:'..bot_id.."userss")
local sendok = 0
for i = 1, #pv do
...
database:srem('Matthew:'..bot_id.."userss",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = database:smembers('Matthew:'..bot_id..'groups') 
local w = 0
local q = 0
for i = 1, #group do
...
database:srem('Matthew:'..bot_id..'pro:groups',group[i]) 
database:srem( 'Matthew:'..bot_id.."groups",group[i]) 
...
local gps = database:smembers('Matthew:'..bot_id.."groups") or 0
local gps2 = database:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("thsake:gog"..bot_id, gps[i])
database:set('Matthew:'..bot_id.."enable:"..gps[i],true)
database:set( 'Matthew:'..bot_id.."charge:"..gps[i],true)
end
...
database:del('Matthew:'..bot_id.."enable:"..gps[i])
database:del( 'Matthew:'..bot_id.."charge:"..gps[i])
end
...
database:del("thsake:gog"..bot_id)
database:del('Matthew:'..bot_id.."groups")
end
------------------------------------------------------------------------
...
if database:get('Matthew:'..bot_id.."group:link"..gpss[i]) then 
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(database:get('Matthew:'..bot_id.."group:link"..gpss[i])).."\n"
else
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link.result or ' لا توجد روابط ').."\n"
end
end
local f = io.open('Matthew_Links.txt', 'w')
f:write(text233)
f:close()
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './Matthew_Links.txt', '', nil)  
end
------------------------------------------------------------------------
...
for i=1, #gpss do
local link = database:get('Matthew:'..bot_id.."group:link"..gpss[i])
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('Matthew.txt', 'w')
f:write(text233)
f:close()
local Matthewe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. Matthewe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'Matthew.txt' .. '"'
io.popen(curl)
end
...
redis:set('Matthew:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'msg')
return false 
end
if text == ("مسح المطورين") then
local list = redis:smembers('Matthew:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('Matthew:'..bot_id..'dev')
redis:del('Matthew:'..bot_id..'sudoo'..v)
end
...
if database:sismember('Matthew:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:sadd('Matthew:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') then
if not database:sismember('Matthew:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:srem('Matthew:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تفعيل الاذاعه') then
if not database:get('Matthew:'..bot_id..'bc:groups') then
send(msg.chat_id_, msg.id_, 1, '✔️┇الاذاعه بالفعل تم تفعيلها', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '✔️┇تم تفعيل اذاعه البوت', 1, 'md')
database:del('Matthew:'..bot_id..'bc:groups')
end
end
if (text and text == 'تعطيل الاذاعه') then
if database:get('Matthew:'..bot_id..'bc:groups') then
send(msg.chat_id_, msg.id_, 1, '🔌┇الاذاعه بالفعل تم تعطيلها', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🔌┇تم تعطيل اذاعه البوت', 1, 'md')
database:set('Matthew:'..bot_id..'bc:groups',true)
end
end
if text and text == "تعين مجموعه المطور" then
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين ~⪼ `'..msg.chat_id_..'`', 1, 'md')
database:set("Matthew"..bot_id..":sudo:gr",msg.chat_id_)
end
if text and text == "مسح مجموعه المطور" then
send(msg.chat_id_, msg.id_, 1, '☑┇ تم مسح مجموعه المطور', 1, 'md')
database:del("Matthew"..bot_id..":sudo:gr")
end
if (text and text == 'استعاده الاوامر') then
redis:del('Matthew:'..bot_id..'help', text)
redis:del('Matthew:'..bot_id..'h1', text)
redis:del('Matthew:'..bot_id..'h2', text)
redis:del('Matthew:'..bot_id..'h3', text)
redis:del('Matthew:'..bot_id..'h4', text)
redis:del('Matthew:'..bot_id..'h5', text)
redis:del('Matthew:'..bot_id..'h6', text)
redis:del('Matthew:'..bot_id..'hres')
send(msg.chat_id_, msg.id_, 1, '☑┇تم استعاده جميع كلايش الاوامر', 1, 'html')
end
if (text and text == 'تغير امر الاوامر') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Matthew:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
return false end


if (text and text == 'تغير امر م1') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Matthew:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
return false end
------------------------------------------------------------------------
...
redis:set('Matthew:'..bot_id..'hres1'..msg.sender_user_id_..'', 'msg')
return false 
end

if (text and text == 'تغير امر م2') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Matthew:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'تغير امر م3') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Matthew:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'تغير امر م4') then
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('Matthew:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end

if (text and text == 'مسح امر المطور بالكليشه') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
redis:del('Matthew:'..bot_id..'text_sudo', text)
...
if (not database:get('Matthew:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("Matthew:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "Matthew"
else
...
if (not database:get('Matthew:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("Matthew:set_if_bc_new:pin"..bot_id..msg.sender_user_id_,"ok")
return "Matthew"
else
send(msg.chat_id_, msg.id_, 1, '☑┇ الاذاعه معطله ', 1, 'md')
end
...
if (not database:get('Matthew:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
database:setex('Matthew:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
...
database:set('Matthew:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("Matthew"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n📌┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
------------------------------------------------------------------------
if text == 'مغادره' then
local lave = database:get('Matthew:'..bot_id..'leave:groups')
print(lave)
if not database:get('Matthew:'..bot_id..'leave:groups') then
...
database:del('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
return false
end
...
return false end
function promote_by_reply(extra, result, success)
database:sadd('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
database:sadd('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
database:sadd('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
...
return false end
function promote_by_reply(extra, result, success)
database:srem('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
function promote_by_username(extra, result, success)
if result.id_ then
database:srem('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
database:srem('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
tsX000(apmd[2],msg,"تم تنزيلة منشئ اساسي من البوت")
return false
end
------------------------------------------------------------------------
if (text == ("عدد الكروبات") or text ==("الاحصائيات")) then
local gps = database:scard('Matthew:'..bot_id.."groups") or 0
local user = database:scard('Matthew:'..bot_id.."userss") or 0
local gps2 = database:scard("thsake:gog"..bot_id) or 0
local gps9 = database:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = database:scard('Matthew:'..bot_id..'pro:groups') or 0
...
local time = a[2] * day
database:setex( 'Matthew:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'Matthew:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'Matthew:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:setex( 'Matthew:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'Matthew:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
------------------------------------------------------------------------
...
database:setex( 'Matthew:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'Matthew:'..bot_id.."charge:"..txt[2],true)
...
local list = database:smembers('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_)
if #list == 0 then
...
local hash =  'Matthew:'..bot_id..'creator:'..msg.chat_id_
...
database:sadd('Matthew:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apow[2] or 'MatthewTEAM')..')\n🔖┇تم رفعة منشئ  في البوت'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @M1_m2s ⚜️\n', 1, 'html')   
return false end
database:sadd('Matthew:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
database:del('Matthew:'..bot_id..'creator:'..msg.chat_id_)
...
local tsX_o = database:get("lock_edit:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل ")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل ")
database:set("lock_edit:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل بالطرد") then
local tsX_o = database:get("lock_edit:Matthew:kick"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التعديل بالطرد")
else
tsX000("lock",msg,"🔐┇تم قفل التعديل بالطرد")
database:set("lock_edit:Matthew:kick"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:Matthew:kick"..msg.chat_id_..bot_id,"ok")
end
end
if text == 'مسح المدراء' then
database:del('Matthew:'..bot_id..'owners:'..msg.chat_id_)
...
local hash =  'Matthew:'..bot_id..'owners:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
database:sadd('Matthew:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
database:sadd('Matthew:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
database:set("Matthew:mute:deleta:msg:"..bot_id..msg.chat_id_,"MatthewE")
send(msg.chat_id_, msg.id_, 1, "🔌┇تم تعطيل خاصيه الابلاغ", 1, 'html')
end
if (text and text == "تفعيل الابلاغ") then 
database:del("Matthew:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
database:del('Matthew:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_)
local user_msgs = database:get('Matthew:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_)
...
database:set('Matthew:'..bot_id.."Matthew:lock:set"..msg.chat_id_,"Matthew")
...
database:del('Matthew:'..bot_id.."Matthew:lock:set"..msg.chat_id_)
...
database:set("Matthew:lock:ban_and_kick"..bot_id..msg.chat_id_,"Matthew")
send(msg.chat_id_, msg.id_, 1, '🔌┇تم تعطيل (طرد - حضر) الاعضاء', 1, 'md')
end
if (text and (text == "تفعيل الطرد" or text == "تفعيل الحظر")) then
database:del("Matthew:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
database:set("Matthew:name_sudo"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------------------
if text and text:match("^تغير رد المنشئ (.*)$") then
ac = {string.match(text, "^تغير رد المنشئ (.*)$")}                     
database:set("Matthew:name_cre"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
...
database:set("Matthew:name_own"..bot_id..msg.chat_id_,ac[1]) 
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
------------------------------------
if text and text:match("^تغير رد الادمن (.*)$") then
ac = {string.match(text, "^تغير رد الادمن (.*)$")}
database:set("Matthew:name_adm"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
if text and text:match("^تغير رد العضو المميز (.*)$") then
ac = {string.match(text, "^تغير رد العضو المميز (.*)$")}
database:set("Matthew:name_vipp"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------
if text and text:match("^تغير رد العضو فقط (.*)$") then
ac = {string.match(text, "^تغير رد العضو فقط (.*)$")}
database:set("Matthew:name_nk"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
...
database:del("Matthew:all_sl:"..text..bot_id..msg.chat_id_)
database:srem('Matthew:'..bot_id..'Matthewnwe:slahea:'..msg.chat_id_,text)
database:del("tsahke:is_del_sl:"..bot_id..msg.chat_id_..msg.sender_user_id_)
end 
if (text and text == "اضف صلاحيه") then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("Matthew:new:sl:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "Matthew"
end
if text and database:get("Matthew:new:sl:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("Matthew:new:sl:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("Matthew:new:sl:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("Matthew:new:sl2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
database:sadd('Matthew:'..bot_id..'Matthewnwe:slahea'..msg.chat_id_,text)
return "Matthew"
end
if (text and database:get("Matthew:new:sl2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
database:del("Matthew:new:sl2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = database:get("Matthew:new:sl:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
database:set("Matthew:all_sl:"..tsha_text..bot_id..msg.chat_id_,text)
database:set("Matthewnwe:slahea:bot"..tsha_text..bot_id..msg.chat_id_,text)
database:del("Matthew:new:sl:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
if text =='مسح الصلاحيات' then 
local list = database:smembers('Matthew:'..bot_id..'Matthewnwe:slahea'..msg.chat_id_)
for k,v in pairs(list) do
local list = database:smembers('Matthew:'..bot_id..'Matthew:users:sl'..msg.chat_id_)
for k,ks in pairs(list) do
ts_text = database:get("Matthew:all_sl:"..v..bot_id..msg.chat_id_)
if ts_text then 
database:del("Matthew:name_user:"..bot_id..msg.chat_id_..ks)
end
end
database:del("Matthew:all_sl:"..v..bot_id..msg.chat_id_)
database:del("Matthewnwe:slahea:bot"..v..bot_id..msg.chat_id_)
end
database:del('Matthew:'..bot_id..'Matthewnwe:slahea'..msg.chat_id_)
...
for k,v in pairs(list) do
var = database:get("Matthewnwe:slahea:bot"..v..bot_id..msg.chat_id_)
if var then
t = t..''..k..'- '..v..' » ('..var..')\n'
else
t = t..''..k..'- '..v..'\n'
...
database:set('Matthew:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
...
database:sadd('Matthew:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
...
database:sadd('Matthew:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
database:srem('Matthew:'..bot_id..'SET:BAN'..msg.chat_id_, result.sender_user_id_)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,by_reply)
end
...
function promote_by_username(extra, result, success)
if result.id_ then
database:srem('Matthew:'..bot_id..'SET:BAN'..msg.chat_id_, result.id_)
...
database:del('Matthew:'..bot_id..'SET:BAN'..msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, '☑┇تم مسح قائمه صلاحيات الحظر', 1, 'md')
end
if text == ("قائمه صلاحيه الحظر") then
local hash =  'Matthew:'..bot_id..'SET:BAN'..msg.chat_id_
local list = database:smembers(hash)
...
local Matthew_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسال الان الامر الجديد", 1, 'md')
database:set("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id,Matthew_edis[1])
database:sadd('Matthew:'..bot_id..'Matthewnwe:'..msg.chat_id_,Matthew_edis[1])
...
database:set("Matthew:edit:text:su:newamr:"..bot_id..msg.chat_id_..database:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("Matthew:edit:text:su:newamr2:"..bot_id..msg.chat_id_..text,database:get("tsahke:new:msg:amr:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
local klma = database:get("Matthew:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
if klma ~= nil then
database:del("Matthew:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
database:del("Matthew:edit:text:su:newamr:"..bot_id..msg.chat_id_..t[1])
database:srem('Matthew:'..bot_id..'Matthewnwe:'..msg.chat_id_,klma)
...
local list = database:smembers('Matthew:'..bot_id..'Matthewnwe:'..msg.chat_id_)
if #list == 0 then
...
for k,v in pairs(list) do
var = database:get("Matthew:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if var then
t = t..''..k..'- '..v..' » ('..var..')\n'
else
t = t..''..k..'- '..v..'\n'
...
local list = database:smembers('Matthew:'..bot_id..'Matthewnwe:'..msg.chat_id_)
for k,v in pairs(list) do
local klma = database:get("Matthew:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
if klma ~= nil then
database:del("Matthew:edit:text:su:newamr2:"..bot_id..msg.chat_id_..klma)
database:del("Matthew:edit:text:su:newamr:"..bot_id..msg.chat_id_..v)
end
end
database:del('Matthew:'..bot_id..'Matthewnwe:'..msg.chat_id_)
...
database:set('Matthew:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @M1_m2s ⚜️\n', 1, 'html')   
return false end
if not is_creator(msg) and database:get('Matthew:'..bot_id.."Matthew:lock:set"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع الرفع تم تعطيل الرفع من قبل المنشئين \n', 1, 'md')
return false
end
function promote_by_reply(extra, result, success)
local hash =  'Matthew:'..bot_id..'mods:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
database:sadd('Matthew:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @M1_m2s ⚜️\n', 1, 'html')   
return false end
database:sadd('Matthew:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
database:del('Matthew:'..bot_id..'mods:'..msg.chat_id_)
...
database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'Matthew')
return "Matthew"
end

if text and text == "مسح الايدي" then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("Matthew:gr:id:text:"..bot_id..msg.chat_id_)
end
...
database:set('Matthew:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
...
database:set('Matthew:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
...
i = i + 1
local user_info = database:hgetall('Matthew:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
if not database:get('Matthew:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '✔️┇ امر اطردني بالفعل تم تفعيلة ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '✔️┇ امر اطردني تم تفعيلة ', 1, 'md')
database:del('Matthew:'..bot_id..'kickme:mute'..msg.chat_id_)
end
end
if (text and text == 'تعطيل اطردني') then
if database:get('Matthew:'..bot_id..'kickme:mute'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🔌┇ تم تعطيل امر اطردني بالفعل ', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🔌┇ تم تعطيل امر اطردني ', 1, 'md')
database:set('Matthew:'..bot_id..'kickme:mute'..msg.chat_id_,true)
end
end
if text and text:match("^رفع (.*)$")  and msg.reply_to_message_id_ then
a = {string.match(text, "^رفع (.*)$")}
ts_text = database:get("Matthew:all_sl:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('Matthew:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:sadd('Matthew:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:sadd('Matthew:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("Matthew:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
database:sadd('Matthew:'..bot_id..'Matthew:users:sl'..msg.chat_id_,result.sender_user_id_)
function keko33332(extra,result2,success)
info = '👤┇العضو ~⪼ ['..result2.first_name_..'](t.me/'..(result2.username_ or 'MatthewTEAM')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
ts_text = database:get("Matthew:all_sl:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
...
database:sadd('Matthew:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:sadd('Matthew:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:sadd('Matthew:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
database:set("Matthew:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
database:sadd('Matthew:'..bot_id..'Matthew:users:sl'..msg.chat_id_,result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MatthewTEAM')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
database:sadd('Matthew:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
database:sadd('Matthew:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:sadd('Matthew:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
database:set("Matthew:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
database:sadd('Matthew:'..bot_id..'Matthew:users:sl'..msg.chat_id_,apmd[2])
...
function demote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:srem('Matthew:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:srem('Matthew:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:srem('Matthew:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:del("Matthew:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
database:srem('Matthew:'..bot_id..'Matthew:users:sl'..msg.chat_id_,result.sender_user_id_)
...
if ts_text == "ادمن" then 
database:srem('Matthew:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:srem('Matthew:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:srem('Matthew:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'MatthewTEAM')..')\n🔰┇تم تنزيله من '..apmd[1]
database:srem('Matthew:'..bot_id..'Matthew:users:sl'..msg.chat_id_,result.id_)
database:del("Matthew:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:sadd('Matthew:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
...
database:sadd('Matthew:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
database:sadd('Matthew:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح لن يتم ارسالها مجددا')  
return false
end
...
if b.content_.sticker_ then
local idsticker = b.content_.sticker_.set_id_
database:srem('Matthew:'..bot_id.."filtersteckr"..msg.chat_id_,idsticker)
text = 'الملصق'
...
if b.content_.ID == "MessagePhoto" then
local photo = b.content_.photo_.id_
database:srem('Matthew:'..bot_id.."filterphoto"..msg.chat_id_,photo)
text = 'الصوره'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
if b.content_.animation_.animation_ then
local idanimation = b.content_.animation_.animation_.persistent_id_
database:srem('Matthew:'..bot_id.."filteranimation"..msg.chat_id_,idanimation)
text = 'المتحركه'
sendtext(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
...
database:set('Matthew:'..bot_id..'lock_geam'..msg.chat_id_,true)  
end
...
database:del('Matthew:'..bot_id..'lock_geam'..msg.chat_id_) 
end
...
local tsX_o = database:get("lock_tag:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التاك ")
else
tsX000("lock",msg,"🔐┇ تم قفل التاك ")
database:set("lock_tag:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الرد") then
local tsX_o = database:get("lock_reple:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الرد على الرسائل ")
else
tsX000("lock",msg,"🔐┇ تم قفل الرد على الرسائل ")
database:set("lock_reple:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الشارحه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الشارحه ")
database:set("lock_sarha:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_lllll:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التكرار ")
else
tsX000("lock",msg,"🔐┇ تم قفل التكرار ")
database:set("lock_lllll:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_files:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الملفات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الملفات ")
database:set("lock_files:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_photo:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_stecker:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_video:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_inline:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانلاين ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانلاين ")
database:set("lock_inline:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدردشه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدردشه ")
database:set("lock_chat:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التوجيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل التوجيه ")
database:set("lock_fwd:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل التثبيت ")
else
tsX000("lock",msg,"🔐┇ تم قفل التثبيت ")
database:set("lock_pin:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_voice:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_contact:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الجهات ")
else
tsX000("lock",msg,"🔐┇ تم قفل الجهات ")
database:set("lock_contact:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل العربيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل العربيه ")
database:set("lock_ar:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الانكليزيه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الانكليزيه ")
database:set("lock_en:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_link:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_username:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل المعرف ")
else
tsX000("lock",msg,"🔐┇ تم قفل المعرف ")
database:set("lock_username:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
local tsX_o = database:get("lock_botAndBan:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات بالطرد ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات بالطرد ")
database:set("lock_botAndBan:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل البوتات ")
else
tsX000("lock",msg,"🔐┇ تم قفل البوتات ")
database:set("lock_bot:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدخول") then
local tsX_o = database:get("lock_join:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الدخول للمجموعه ")
else
tsX000("lock",msg,"🔐┇ تم قفل الدخول للمجموعه ")
database:set("lock_join:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
...
--         »»                 End Matthew lock                         ««              --
--         »»               Start Matthew unlock                       ««              --
if (text == "فتح الكل") then
database:del("lock_tag:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_sarha:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_word:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_lllll:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_files:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_mark:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_contact:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_fwd:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_link:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_username:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_botAndBan:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_new:Matthew"..msg.chat_id_..bot_id,"ok")
...
database:del("lock_reple:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:Matthew"..msg.chat_id_..bot_id)
end
end
...
database:del("lock_inline:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:Matthew"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_join:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
if text and text:match('^وضع عدد الكانسر (%d+)$') then
local Num = text:match('^وضع عدد الكانسر (%d+)$')
database:set('Matthew:'..bot_id..'Num:kansers'..msg.chat_id_,Num)
send(msg.chat_id_, msg.id_, 1, '\n🔖┇تم وضع عدد حروف الاسم {'..Num..'} حرف',1, 'md')
end
if (text == "قفل الكانسر") then
local tsX_o = database:get("lock_kansers:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔐┇ بالفعل تم قفل الكانسر ")
else
tsX000("lock",msg,"🔐┇ تم قفل الكانسر ")
database:set("lock_kansers:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_kansers:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
...
return false end
function promote_by_reply(extra, result, success)
local hash =  'Matthew:'..bot_id..'vipgp:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if not is_creator(msg) and not is_setban(msg) and database:get("Matthew:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
function ban_by_reply(extra, result, success)
local hash =  'Matthew:'..bot_id..'banned:'..msg.chat_id_
...
database:sadd('Matthew:'..bot_id..'banned:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'MatthewTEAM')..')\n☑┇تم حظره من المجموعه'
...
database:sadd('Matthew:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
local hash =  'Matthew:'..bot_id..'banned:'..msg.chat_id_
local hash1 =  'Matthew:'..bot_id..'banned:'..msg.chat_id_
...
database:del('Matthew:'..bot_id..'banned:'..msg.chat_id_, result.id_)
database:del('Matthew:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apba[2] or 'MatthewTEAM')..')\n☑┇تم تحريره من القيود'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
database:del('Matthew:'..bot_id..'banned:'..msg.chat_id_, apba[2])
database:del('Matthew:'..bot_id..'muted:'..msg.chat_id_, apba[2])
...
database:set('Matthew:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
database:srem('Matthew:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:srem('Matthew:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
function mute_by_reply(extra, result, success)
local hash =  'Matthew:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) == true then
...
database:sadd('Matthew:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apsi[2] or 'MatthewTEAM')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
...
database:sadd('Matthew:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('Matthew:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('Matthew:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apki[2] or 'MatthewTEAM')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
...
local Matthew_098 = 0
if database:get("Matthew:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("Matthew:get:mod:"..bot_id..msg.chat_id_)
Matthew_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..Matthew_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
database:set("Matthew:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:set('Matthew:'..bot_id.."group:link"..msg.chat_id_,text)
...
local wel = database:get('Matthew:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
Matthew_0 = {string.match(text, "^(تعين عدد الاحرف) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين عدد الاحرف {`'..Matthew_0[2]..'`}', 1, 'md')
database:set("Matthew:not:word:"..bot_id..msg.chat_id_,Matthew_0[2])
end

if text == ("الاعدادات") then
if database:get("lock_media:Matthew"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('Matthew:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('Matthew:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not database:get( 'Matthew:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'Matthew:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:Matthew"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_bot:Matthew"..msg.chat_id_..bot_id) then
mute_bots = '✔┇'
else
mute_bots = '✖┇'
end
...
if database:get("lock_lllll:Matthew"..msg.chat_id_..bot_id) then
mute_flood = '✔┇'
else
mute_flood = '✖┇'
end
------------
if database:get("lock_inline:Matthew"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:Matthew"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:Matthew"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if database:get('Matthew:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
...
------------
local ex = database:ttl( 'Matthew:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
local list = database:smembers('Matthew:'..bot_id.."repmedia"..msg.chat_id_)  
for k,v in pairs(list) do  
database:del('Matthew:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('Matthew:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)  
database:srem('Matthew:'..bot_id.."repmedia"..msg.chat_id_,v)  
end  
...
for k,v in pairs(list) do  
local taha = database:get('Matthew:'..bot_id.."filtr1:add:reply:rd"..v..msg.chat_id_)   
t = t..''..k..'- '..v..' » {'..taha..'}\n'    
end  
if #list == 0 then  
...
database:set('Matthew:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"rep")  
return false  
end    
if text then   
local tsssst = database:get('Matthew:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
send(msg.chat_id_, msg.id_, 1, "📥┇ ارسل تحذير عند ارسال الكلمه \n", 1, 'md')  
database:set('Matthew:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"repp")  
database:set('Matthew:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_, text)  
database:sadd('Matthew:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
if text then  
local test = database:get('Matthew:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
if test == 'repp' then  
send(msg.chat_id_, msg.id_, 1, '🔖┇تم منع الكلمه لن يتم ارسالها بعد الان  \n', 1, 'md')  
database:del('Matthew:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
local test = database:get('Matthew:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
if text then   
database:set('Matthew:'..bot_id.."filtr1:add:reply:rd"..test..msg.chat_id_, text)  
database:hset('Matthew:'..bot_id..'filters:'..msg.chat_id_, text, 'filtered')
end  
database:del('Matthew:'..bot_id.."filtr1:add:reply2"..msg.sender_user_id_..msg.chat_id_)  
return false  end  
end
...
database:set('Matthew:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = database:get('Matthew:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
...
database:del('Matthew:'..bot_id.."filtr1:add:reply1"..msg.sender_user_id_..msg.chat_id_)  
database:del('Matthew:'..bot_id.."filtr1:add:reply:rd"..text..msg.chat_id_)  
database:srem('Matthew:'..bot_id.."repmedia"..msg.chat_id_,text)  
return false  end  
end
...
database:set('Matthew:'..bot_id..'rules'..msg.chat_id_, txt[2])
...
database:set('Matthew:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
local hash = 'Matthew:'..bot_id..'res:'..msg.chat_id_
local list = database:smembers(hash)
text = "👥┇قائمة المقيدين ،\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
local user_info = database:hgetall('Matthew'..bot_id.."user:"..v)
...
local hash = 'Matthew:'..bot_id..'res:'..msg.chat_id_
local list = database:smembers(hash) 
for k,v in pairs(list) do database:del('Matthew:'..bot_id..'res:'..msg.chat_id_) 
...
database:sadd('Matthew:'..bot_id..'res:'..msg.chat_id_, result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(res[2] or 'Matthewteam')..')\n☑️┇تم تقييده'
end
else
texts = '✖️┇خطاء'
end
...
database:sadd('Matthew:'..bot_id..'res:'..msg.chat_id_, apba[2])
...
local tsX_o = database:get("lock_tag.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التاك بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التاك بالتقييد")
database:set("lock_tag.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الشارحه بالتقيد") then
local tsX_o = database:get("lock_sarha.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الشارحه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الشارحه بالتقييد")
database:set("lock_sarha.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المتحركه بالتقيد") then
local tsX_o = database:get("lock_gif.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل المتحركه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المتحركه بالتقييد")
database:set("lock_gif.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملفات بالتقيد") then
local tsX_o = database:get("lock_files.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملفات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملفات بالتقييد")
database:set("lock_files.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الماركدون بالتقيد") then
local tsX_o = database:get("lock_mark.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الماكدون بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الماركدون بالتقييد")
database:set("lock_mark.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصور بالتقيد") then
local tsX_o = database:get("lock_photo.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصور بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصور بالتقييد")
database:set("lock_photo.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الملصقات بالتقيد") then
local tsX_o = database:get("lock_stecker.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الملصقات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الملصقات بالتقييد")
database:set("lock_stecker.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الفيديو بالتقيد") then
local tsX_o = database:get("lock_video.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الفيديو بالتقييد")
database:set("lock_video.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانلاين بالتقيد") then
local tsX_o = database:get("lock_inline.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانلاين بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانلاين بالتقييد")
database:set("lock_inline.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الدردشه بالتقيد") then
local tsX_o = database:get("lock_chat.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الدردشه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الدردشه بالتقييد")
database:set("lock_chat.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل التوجيه بالتقيد") then
local tsX_o = database:get("lock_fwd.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل التوجيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل التوجيه بالتقييد")
database:set("lock_fwd.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الاغاني بالتقيد") then
local tsX_o = database:get("lock_audeo.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الاغاني بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الاغاني بالتقييد")
database:set("lock_audeo.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الصوت بالتقيد") then
local tsX_o = database:get("lock_voice.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الصوت بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الصوت بالتقييد")
database:set("lock_voice.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الجهات بالتقيد") then
local tsX_o = database:get("lock_contact.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الجهات بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الجهات بالتقييد")
database:set("lock_contact.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل العربيه بالتقيد") then
local tsX_o = database:get("lock_ar.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل العربيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل العربيه بالتقييد")
database:set("lock_ar.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الانكليزيه بالتقيد") then
local tsX_o = database:get("lock_en.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الانكليزيه بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الانكليزيه بالتقييد")
database:set("lock_en.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل الروابط بالتقيد") then
local tsX_o = database:get("lock_link.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل الروابط بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل الروابط بالتقييد")
database:set("lock_link.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل المعرف بالتقيد") then
local tsX_o = database:get("lock_username.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
 tsX000("lock",msg,"☑️┇بالفعل تم قفل المعرف بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل المعرف بالتقييد")
database:set("lock_username.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
if (text == "قفل بصمه الفيديو بالتقيد") then
local tsX_o = database:get("lock_note.note:Matthew"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☑️┇بالفعل تم قفل بصمه الفيديو بالتقييد")
else
tsX000("lock",msg,"☑️┇تم قفل بصمه الفيديو بالتقييد")
database:set("lock_note.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_tag.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_sarha.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_gif.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_files.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_mark.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_photo.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_stecker.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_video.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_inline.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_chat.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_fwd.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_audeo.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_voice.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_contact.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_ar.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_en.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_username.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_note.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
database:del("lock_link.note:Matthew"..msg.chat_id_..bot_id,"ok")
end
end
------------------------------------------------------------------------
...
if database:get("lock_audeo.note:Matthew"..msg.chat_id_..bot_id) then
 ...
 ------------
if database:get("lock_inline.note:Matthew"..msg.chat_id_..bot_id) then
 mute_in = '✔┇'
else
 mute_in = '✖┇'
 end
...
lock_tag = '✖┇'
 ...
 ------------
if database:get("lock_contact.note:Matthew"..msg.chat_id_..bot_id) then
lock_contact = '✔┇'
else
lock_contact = '✖┇'
 end
...
database:del('Matthew:'..bot_id..'banned:'..msg.chat_id_)
...
if database:sismember('Matthew:'..bot_id..'gbanned:',result.id_) then
kewd = 'محظور عام'
elseif database:sismember('Matthew:'..bot_id..'banned:'..msg.chat_id_,result.id_) then
kewd = 'محظور'
elseif database:sismember('Matthew:'..bot_id..'muted:'..msg.chat_id_,result.id_) then
kewd = 'مكتوم'
elseif database:sismember('Matthew:'..bot_id..'res'..msg.chat_id_,result.id_) then
kewd = 'مقيد'
else
kewd = ' لا يوجد'
end
...
if redis:sismember('Matthew:'..bot_id..'dev',result.id_) then
dev = 'المطور ،' else dev = '' end
if database:sismember('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if database:sismember('Matthew:'..bot_id..'creator:'..msg.chat_id_, result.id_) then
cr = 'منشئ ،' else cr = '' end
if database:sismember('Matthew:'..bot_id..'owners:'..msg.chat_id_, result.id_) then
own = 'مدير ،' else own = '' end
if database:sismember('Matthew:'..bot_id..'mods:'..msg.chat_id_, result.id_) then
mod = 'ادمن ،' else mod = '' end
if database:sismember('Matthew:'..bot_id..'vipgp:'..msg.chat_id_, result.id_) then
vip = 'مميز ،' else vip = ''
end
...
database:srem('Matthew:'..bot_id..'dev', result.id_)
database:del('Matthew:'..bot_id..'sudoo'..result.id_..'', 'no')
database:srem('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_,result.id_)
database:srem('Matthew:'..bot_id..'creator:'..msg.chat_id_, result.id_)
database:srem('Matthew:'..bot_id..'owners:'..msg.chat_id_, result.id_)
database:srem('Matthew:'..bot_id..'mods:'..msg.chat_id_, result.id_)
database:srem('Matthew:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
if redis:sismember('Matthew:'..bot_id..'dev',result.sender_user_id_) then
dev = 'المطور ،' else dev = '' end
if database:sismember('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if database:sismember('Matthew:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_) then
cr = 'منشئ ،' else cr = '' end
if database:sismember('Matthew:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_) then
own = 'مدير ،' else own = '' end
if database:sismember('Matthew:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_) then
mod = 'ادمن ،' else mod = '' end
if database:sismember('Matthew:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_) then
vip = 'مميز ،' else vip = ''
end
...
if RemBot(msg.sender_user_id_,msg.chat_id_) == 'sudo1' then
database:srem('Matthew:'..bot_id..'dev', result.sender_user_id_)
database:del('Matthew:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
database:srem('Matthew:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
database:srem('Matthew:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
database:srem('Matthew:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('Matthew:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('Matthew:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif RemBot(msg.sender_user_id_,msg.chat_id_) == 'sudo' then
...
------------------------------------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("Matthew:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("Matthew:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("Matthew:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
...
t = database:get("Matthew:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg) then
t = database:get("Matthew:name_adm"..bot_id..msg.chat_id_) or ' ادمن الكروب خالي '
elseif is_vip(msg) then
t = database:get("Matthew:name_vipp"..bot_id..msg.chat_id_) or 'عضو مميز '
else
t = database:get("Matthew:name_nk"..bot_id..msg.chat_id_) or 'عضو محد يحترمك' 
end
...
if not database:get('Matthew:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('Matthew:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_) or 0)
if tonumber((database:get('Matthew:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Matthew:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_))
end
local msgs = ((database:get('Matthew:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0) + tahna)
local edit = database:get('Matthew:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
...
if msg.sender_user_id_ == tonumber(sudo_add) then
Matthew_oop = 'المطور الاساسي'
elseif is_sudo(msg2) then
Matthew_oop = database:get("Matthew:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت '
elseif is_creatorbasic(msg) then
Matthew_oop = database:get("Matthew:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي '
elseif (database:get("Matthew:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("Matthew:all_sl:"..database:get("Matthew:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
Matthew_oop = database:get("Matthew:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
Matthew_oop = database:get("Matthew:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب '
elseif (database:get("Matthew:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("Matthew:all_sl:"..database:get("Matthew:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
Matthew_oop = database:get("Matthew:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
Matthew_oop = database:get("Matthew:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب '
elseif is_mod(msg2) then
Matthew_oop = database:get("Matthew:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب'
elseif is_vip(msg2) then
Matthew_oop = database:get("Matthew:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز '
else
Matthew_oop = database:get("Matthew:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط ' 
end
...
if result.id_ then
if not database:get('Matthew:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('Matthew:'..bot_id..'nummsg'..msg.chat_id_..result.id_) or 0)
if tonumber((database:get('Matthew:'..bot_id..'add:numall'..msg.chat_id_..result.id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Matthew:'..bot_id..'add:numall'..msg.chat_id_..result.id_))
end
local msgs = ((database:get('Matthew:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local edit = database:get('Matthew:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = ((database:get('Matthew:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
ikeko_text = "غير متفاعل"
...
------------------------------------------------------------------------
if text == ("الرابط") then
if not database:get("Matthew:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if (database:get('Matthew:'..bot_id.."group:link"..msg.chat_id_) and database:get('Matthew:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '🔘┇ *Group Link* \n['..database:get('Matthew:'..bot_id.."group:link"..msg.chat_id_)..']', 1, "md")
...
database:set('Matthew:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
local user_msgs = database:get('Matthew:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (database:get('Matthew:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if not database:get('Matthew:'..bot_id..'id:mute'..msg.chat_id_) then
...
------------------------------------------------------------------------
if text == ("جهاتي") then
add = (tonumber(database:get('Matthew:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('Matthew:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
...
database:del('Matthew:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
local Matthew = '🗑 ┇ تم حذف سحكاتك '
send(msg.chat_id_, msg.id_, 1, Matthew, 1, 'md')
end
------------------------------------------------------------------------
if text == ("اطردني") then
if not database:get('Matthew:'..bot_id..'kickme:mute'..msg.chat_id_) then
redis:set('Matthew:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:set('Matthew:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
local yess = redis:get('Matthew:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('Matthew:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('Matthew:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🕺🏿┇ لقد تم طردك   ', 1, 'md')
end
end
end
if text and text:match("^لا$") then
local noo = redis:get('Matthew:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('Matthew:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('Matthew:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
------------------------------------------------------------------------
if (text and text == (redis:get('Matthew:'..bot_id..'name_bot') or 'ماثيو'))then
name_bot = (redis:get('Matthew:'..bot_id..'name_bot') or 'ماثيو')
local namebot = {
...
local text_sudo = redis:get('Matthew:'..bot_id..'text_sudo')
local nkeko = redis:get('Matthew:'..bot_id..'nmkeko')
local nakeko = redis:get('Matthew:'..bot_id..'nakeko')
if text_sudo then
...
local rules = database:get('Matthew:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
if not database:sismember('Matthew:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي') then
database:sadd('Matthew:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي')
local keko_info = nil
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
local function getpro(extra, result, success)
local all_photo_Matthew = result.total_count_ 
local tahna = (database:get('Matthew:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if tonumber((database:get('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))
end
local user_msgs = ((database:get('Matthew:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
...
t = database:get("Matthew:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t =  database:get("Matthew:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (database:get("Matthew:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("Matthew:all_sl:"..database:get("Matthew:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("Matthew:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("Matthew:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🏻'   
...
if not database:get('Matthew:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('Matthew:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("Matthew:gr:id:text:"..bot_id..msg.chat_id_) then 
local keko_text = {
'صورتك فدشي 😘😔❤️  ',
"صارلك شكد مخليه ",
"وفالله 😔💘",
...
local Matthew_new_text = database:get("Matthew:gr:id:text:"..bot_id..msg.chat_id_)
local Matthew_new_text = Matthew_new_text:gsub('#username',(keko_info or 'لا يوجد'))
local Matthew_new_text = Matthew_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local Matthew_new_text = Matthew_new_text:gsub('#edit',(edit or 'لا يوجد'))
local Matthew_new_text = Matthew_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local Matthew_new_text = Matthew_new_text:gsub('#stast',(t or 'لا يوجد'))
local Matthew_new_text = Matthew_new_text:gsub('#auto',(ikeko_text or 'لا يوجد'))
local Matthew_new_text = Matthew_new_text:gsub('#photos',(all_photo_Matthew or 'لا يوجد'))
local Matthew_new_text = Matthew_new_text:gsub('#game',(nko or 'لا يوجد'))
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,Matthew_new_text,msg.id_,msg.id_.."")
end
else
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
...
local Matthew_new_text = Matthew_new_text:gsub('#photos',(all_photo_Matthew or 'لا يوجد'))
local Matthew_new_text = Matthew_new_text:gsub('#game',(nko or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, Matthew_new_text, 1, 'html')
end   
end
else
end
...
------------------------------------------------------------------------
if text then
if database:sismember('Matthew:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,text) then
else
database:del('Matthew:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_) 
end
end
...
send(msg.chat_id_, msg.id_, 1, '📜┇معرف ~⪼ ('..(result.username_..')\n🔘┇الاسم ~⪼ ('..result.first_name_ or "Matthew")..')', 1, 'md')
end
getUser(memb[2],whois)
end
------------------------------------------------------------------------
if text == ("مشاهده منشور") then
database:set('Matthew:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
------------------------------------------------------------------------
if text == 'سمايلات' and database:get('Matthew:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Matthew:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🥖','🥐','🍞','🥨','🍟','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍠','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
database:set('Matthew:'..bot_id..'klmos'..msg.chat_id_,name)
taha = '🚀┇اسرع واحد يدز » {`'..name..'`}'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
------------------------------------------------------------------------
if text == ''..(database:get('Matthew:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Matthew:'..bot_id..'l:ids'..msg.chat_id_) then
if not database:get('Matthew:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل سمايلات ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Matthew:'..bot_id..'l:ids'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'الاسرع' and database:get('Matthew:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Matthew:'..bot_id..'l:id'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Matthew:'..bot_id..'klmo'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('Matthew:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Matthew:'..bot_id..'l:id'..msg.chat_id_) then
if not database:get('Matthew:'..bot_id..'l:id'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل الاسرع ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1) 
database:incrby('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Matthew:'..bot_id..'l:id'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'حزوره' and database:get('Matthew:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Matthew:'..bot_id..'l:id1'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Matthew:'..bot_id..'klmoa'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('Matthew:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Matthew:'..bot_id..'l:id1'..msg.chat_id_) then
if not database:get('Matthew:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل حزوره ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Matthew:'..bot_id..'l:id1'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'المعاني' and database:get('Matthew:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Matthew:'..bot_id..'l:id2'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('Matthew:'..bot_id..'means'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('Matthew:'..bot_id..'means'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Matthew:'..bot_id..'l:id2'..msg.chat_id_) then
if not database:get('Matthew:'..bot_id..'l:id2'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل المعاني ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Matthew:'..bot_id..'l:id2'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if text == 'العكس' and database:get('Matthew:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('Matthew:'..bot_id..'l:id3'..msg.chat_id_)
katu = {'باي','فهمت','موزين','اسمعك','احبك','موحلو','نضيف','حاره','ناصي','جوه','سريع','ونسه','طويل','سمين','ضعيف','شريف','شجاع','رحت','عدل','نشيط','شبعان','موعطشان','خوش ولد','اني','هادئ'}
name = katu[math.random(#katu)]
database:set('Matthew:'..bot_id..'aks'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == ''..(database:get('Matthew:'..bot_id..'aks'..msg.chat_id_) or 'لفاتع')..'' and not database:get('Matthew:'..bot_id..'l:id3'..msg.chat_id_) then
if not database:get('Matthew:'..bot_id..'l:id3'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n🎮| للعب مره اخرى ارسل العكس ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('Matthew:'..bot_id..'l:id3'..msg.chat_id_,true)
end 
------------------------------------------------------------------------
if database:get('Matthew:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('Matthew:'..bot_id.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('Matthew:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('Matthew:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
database:incrby('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 5)  
database:incrby('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 5)    
taha = '*🔖┇ مبروك فزت ويانه وخمنت الرقم الصحيح\n🚸┇ تم اضافة { 5 } من النقاط *\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:incrby('Matthew:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(database:get('Matthew:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
database:del('Matthew:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('Matthew:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------
if database:get('Matthew:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('Matthew:'..bot_id.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('Matthew:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
taha = '*📮┇ مبروك فزت وطلعت المحيبس بل ايد رقم { '..NUM..' }\n🎊┇ لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل *'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 3)  
database:incrby('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 3)    
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:del('Matthew:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
------------------------------------------------------------------------
if text == 'المختلف' and database:get('Matthew:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','✨','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧜‍♂','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕒','🕤','⌛️','📅',
};
name = katu[math.random(#katu)]
database:del('Matthew:'..bot_id..'l:idss'..msg.chat_id_)
database:set('Matthew:'..bot_id..':Set_alii:'..msg.chat_id_,name)
...
------------------------------------------------------------------------
if text == 'امثله' and database:get('Matthew:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {
'جوز','ضراطه','الحبل','الحافي','شقره','بيدك','سلايه','النخله','الخيل','حداد','المبلل','يركص','قرد','العنب','العمه','الخبز','بالحصاد','شهر','شكه','يكحله',
};
name = katu[math.random(#katu)]
database:set('Matthew:'..bot_id..':Set_Amthlh:'..msg.chat_id_,name)
database:del('Matthew:'..bot_id..'l:idst'..msg.chat_id_)
...
------------------------------------------------------------------------
if text == ''..(database:get('Matthew:'..bot_id..':Set_alii:'..msg.chat_id_) or '57999')..'' then -- // المختلف
if not database:get('Matthew:'..bot_id..'l:idss'..msg.chat_id_) then 
database:incrby('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('Matthew:'..bot_id..':Set_alii:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
database:set('Matthew:'..bot_id..'l:idss'..msg.chat_id_,true)
end
------------------------------------------------------------------------
if text == ''..(database:get('Matthew:'..bot_id..':Set_Amthlh:'..msg.chat_id_) or '6499')..'' then -- // امثله
if not database:get('Matthew:'..bot_id..'l:idst'..msg.chat_id_) then 
database:incrby('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('Matthew:'..bot_id..':Set_Amthlh:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end
database:set('Matthew:'..bot_id..'l:idst'..msg.chat_id_,true)
end
------------------------------------------------------------------------
if text == 'محيبس' or text == 'بات' then
if database:get('Matthew:'..bot_id..'lock_geam'..msg.chat_id_) then   
Num = math.random(1,6)
database:set('Matthew:'..bot_id.."GAMES"..msg.chat_id_,Num) 
TEST = [[
*➀       ➁     ➂      ➃      ➄     ➅
↓      ↓     ↓      ↓     ↓     ↓
👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊
...
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('Matthew:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
------------------------------------------------------------------------
...
database:set('Matthew:'..bot_id.."GAMES:NUM"..msg.chat_id_,Num) 
TEST = '*\n🎮┇ اهلا بك عزيزي في لعبة التخمين :\nٴ━━━━━━━━━━\n'..'⚠┇ ملاحظه لديك { 3 } محاولات فقط فكر قبل ارسال تخمينك \n\n'..'🔖┇ سيتم تخمين عدد ما بين ال {1 و 20} اذا تعتقد انك تستطيع الفوز جرب واللعب الان ؟ \n💥*'
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('Matthew:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
------------------------------------------------------------------------
if text =='مجوهراتي' then 
if tonumber((database:get('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💎┇ ليس لديك مجوهرات \n📬┇ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎┇ عدد مجوهراتك الحاليه  ('..(database:get('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎┇ مجموع مجوهراتك  ('..(database:get('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
taha = (database:get('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(database:get('Matthew:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
database:incrby('Matthew:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
database:del('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((database:get('Matthew:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
...
database:del('Matthew:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)  
database:del('Matthew:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_)  
send(msg.chat_id_, msg.id_, 1, "تم حذف جميع مجوهراتك", 1, "md") 
end
------------------------------------------------------------------------
if text == 'حذف رسائلي' then
database:del('Matthew:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_)
database:del('Matthew:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, "🗑 ┇ تم حذف رسائلك  ", 1, "md") 
end
------------------------------------------------------------------------
if text == ("الاوامر") and (is_mod(msg) or is_creatorbasic(msg)) then
local help = redis:get('Matthew:'..bot_id..'help')
local text =  [[
📮┇هناك {4} اوامر لعرضها
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
local h1 = redis:get('Matthew:'..bot_id..'h1')
local text =  [[
...
local h2 = redis:get('Matthew:'..bot_id..'h2')
local text =  [[
🥈┇اوامر الادمنيه
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
🚫┇كتم
...
local h3 = redis:get('Matthew:'..bot_id..'h3')
local text =  [[
🥇┇ اوامر المدراء
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
local hres = redis:get('Matthew:'..bot_id..'hres')
local text =[[
...
local h4 = redis:get('Matthew:'..bot_id..'h4')
local text =  [[
🎖┇اوامر المطور
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✳️┇تفعيل/تعطيل
...
📜┇[قناه شروحات سورس](http://t.me/MatthewTEAM)

📥┇[قناه ملفات السورس](t.me/MatthewTEAM)
...
database:del( 'Matthew:'..bot_id.."charge:"..'-100'..data.channel_.id_)
database:srem("thsake:gog"..bot_id, '-100'..data.channel_.id_)
database:sadd( 'Matthew:'..bot_id.."groups",'-100'..data.channel_.id_)
end
end
local chat = {}
if (data.ID == "UpdateNewMessage") then
local Data_Matthew = data
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data) 
if data.username_ ~= false then
database:hset('Matthew:'..bot_id..'user:'..msg.sender_user_id_,'username',(data.username_ or msg.sender_user_id_))
...
local Bots = database:get("lock_bot:Matthew"..msg.chat_id_..bot_id)
for i=0,#mem_id do  
...
if msg.content_.ID == "MessageChatAddMembers" then  
local mem_id = msg.content_.members_  
local Bots = database:get("lock_botAndBan:Matthew"..msg.chat_id_..bot_id) 
for i=0,#mem_id do  
if msg.content_.members_[i].type_.ID == "UserTypeBot" and not is_mod(msg) and Bots then   
...
if database:get("Matthew:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local Matthew_edit_text = database:get("Matthew:edit:text:su:newamr2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
print(Matthew_edit_text)
if Matthew_edit_text then
data.message_.content_.text_ = (Matthew_edit_text or data.message_.content_.text_)
end
end
end
local Name_Bot = (redis:get('Matthew:'..bot_id..'name_bot') or 'ماثيو')
...
------------------------------------------------------------------------------------------------------------------------------------------------
Matthew_run_(msg,data)
Matthew_run_file(Data_Matthew)
------------------------------------------------------------------------
--         »»                 Start UpdateChat                         ««              --
...
local msgg = result 
database:incr('Matthew:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
if database:get("lock_edit:Matthew"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
...
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/MatthewTEAM)" , 1, 'md') 
end
end
getUser(result.sender_user_id_,get_edit)
delete_msg(msg.chat_id_,{[0] = msg.message_id_}) 
elseif database:get("lock_edit:Matthew:kick"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) and not is_creatorbasic(msgg) then
function get_edit(arg,data)
local username = data.username_
local name = data.first_name_
local iduser = data.id_
...
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/MatthewTEAM)\n🚷┇ وتم طرده \n" , 1, 'md') 
end
...
------------------------------------------------------------------------
local taha = database:get('Matthew:'..bot_id.."add:reply:rd"..text..msg.chat_id_)   
...
------------------------------------------------------------------------
--         »»                 End UpdateChat                          ««              --
elseif (data.ID == "UpdateOption" and data.name_ == "my_id") then  local list = database:smembers('Matthew:'..bot_id.."userss") for k,v in pairs(list) do tdcli_function({ID='GetChat',chat_id_ = v},function(arg,data) end,nil) end 
local list = database:smembers('Matthew:'..bot_id..'groups') 
for k,v in pairs(list) do 
...
database:srem('Matthew:'..bot_id..'pro:groups',v) 
database:srem( 'Matthew:'..bot_id.."groups",v) 
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
database:srem("thsake:gog"..bot_id,v) 
database:srem('Matthew:'..bot_id..'pro:groups',v) 
...
database:sadd( 'Matthew:'..bot_id.."groups",v) 
...
local msg = data.message_
local text = msg.content_.text_
local Get_Msg_Pin = database:get('Matthew:'..bot_id..'Msg_Text'..msg.chat_id_)
if Get_Msg_Pin ~= nil then
if text == Get_Msg_Pin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) if d.ID == 'Ok' then;database:del('Matthew:'..bot_id..'Msg_Text'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
print(msg.content_.sticker_.sticker_.persistent_id_)
if Get_Msg_Pin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) database:del('Matthew:'..bot_id..'Msg_Text'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.voice_) then
